...
void EventManager<EventName, Event>::touch(const EventName &name) {
    _events.emplace(
        name, EventInfo{ .type = std::type_index(typeid(SpecificEvent)) });
}

template <typename EventName, typename Event>
void EventManager<EventName, Event>::touch(const EventName &name) {
    _events.emplace(name, EventInfo{ .type = std::type_index(typeid(Event)) });
}
...
void EventManager<EventName, Event>::_checkType(const EventName &name) {
    ...
    if (_events.at(name).type != std::type_index(typeid(SpecificEvent))) {
        throw std::runtime_error(
            "Wrong type of event: " +
            std::string(typeid(SpecificEvent).name()) + " (expected " +
            std::string(_events.at(name).type.name()) + ")");
    }
}

template <typename EventName, typename Event>
template <typename SpecificEvent, typename>
CallbackHandle EventManager<EventName, Event>::on(
...
template <typename EventName, typename Event>
CallbackHandle EventManager<EventName, Event>::on(
    const EventName &name, const std::function<void(const Event *)> &callback) {
...
template <typename EventName, typename Event>
void EventManager<EventName, Event>::off(
    const EventName &name, const CallbackHandle &handle) {
    auto &event_callbacks = _events.at(name).callbacks;
...
template <typename EventName, typename Event>
template <typename SpecificEvent, typename>
void EventManager<EventName, Event>::emit(
    const EventName &name, const SpecificEvent &event) {
    _checkType<SpecificEvent>(name);
...
template <typename EventName, typename Event>
void EventManager<EventName, Event>::emit(
    const EventName &name, const Event &event) {
    _checkType<Event>(name);
