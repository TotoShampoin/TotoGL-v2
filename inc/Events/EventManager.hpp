...
class EventManager {
    static_assert(
        !std::is_arithmetic<Event>::value, "Event cannot be a primitive type");
...
public:
    ...
    CallbackHandle
    on(const EventName &, const std::function<void(const SpecificEvent *)> &);
    CallbackHandle
    on(const EventName &, const std::function<void(const Event *)> &);
