...
public:
    BasicMaterial(const ColorRGB& color = ColorRGB(1.), const std::shared_ptr<Texture>& texture = nullptr);
...
private:
    std::shared_ptr<ShaderMaterial> _shader = ShaderMaterial::create( simple_vertex, basic_fragment );
    ColorRGB _color;
