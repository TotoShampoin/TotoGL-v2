...
public:
    ...
    static std::shared_ptr<Object> create() { return std::make_shared<Object>(); }
    ...
    template <typename SubObject, typename = std::enable_if_t<std::is_base_of<Object, SubObject>::value>>
    static std::vector<std::shared_ptr<SubObject>> filter(const std::vector<std::shared_ptr<Object>>&);
    ...
    const std::vector<std::shared_ptr<Object>>& children() const { return _children; }
...
protected:
    ...
    void updateMatrixWorld(const Matrix4& parent) { _matrix_world = matrix() * parent; }
private:
    Vector3 _position {0};
    Vector3 _scale {1};
    Euler _rotation;
