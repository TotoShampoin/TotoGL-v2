...
public:
    Geometry();
    Geometry(const std::vector<Vertex>& verts, const std::vector<Triangle>& tris, const std::vector<size_t>& mats = {});
    ~Geometry();
    static std::shared_ptr<Geometry> create(const std::vector<Vertex>& verts, const std::vector<Triangle>& tris, const std::vector<size_t>& mats = {}) {
...
struct PlaneGeometry : Geometry {
    ...
    static std::shared_ptr<PlaneGeometry> create(const float& width = 1, const float& height = 1) {
        return std::make_shared<PlaneGeometry>(width, height);
    }
};
struct BoxGeometry : Geometry {
    BoxGeometry(const float& width = 1, const float& height = 1, const float& depth = 1);
    static std::shared_ptr<BoxGeometry> create(const float& width = 1, const float& height = 1, const float& depth = 1) {
        return std::make_shared<BoxGeometry>(width, height, depth);
    }
};
struct SphereGeometry : Geometry {
    SphereGeometry(const float& radius = 1, const int& width_segments = 32, const int& height_segments = 16);
    static std::shared_ptr<SphereGeometry> create(const float& radius = 1, const int& width_segments = 32, const int& height_segments = 16) {
        return std::make_shared<SphereGeometry>(radius, width_segments, height_segments);
    }
};
struct CylinderGeometry : Geometry {
    CylinderGeometry(
        const float& radius_top = 1, const float& radius_bottom = 1, const float& height = 1,
        const int& radial_segments = 32, const int& height_segments = 1,
...
// std::shared_ptr<Geometry> PlaneGeometry(const float& width = 1, const float& height = 1);
// std::shared_ptr<Geometry> BoxGeometry(const float& width = 1, const float& height = 1, const float& depth = 1);
// std::shared_ptr<Geometry> SphereGeometry(const float& radius = 1, const int& width_segments = 32, const int& height_segments = 16);
// std::shared_ptr<Geometry> CylinderGeometry(
//     const float& radius_top = 1, const float& radius_bottom = 1, const float& height = 1,
//     const int& radial_segments = 32, const int& height_segments = 1,
//     const bool& open_ended = false);
