...
std::vector<std::shared_ptr<SubObject>> Object::filter(const std::vector<std::shared_ptr<Object>>& objects) {
    std::vector<std::shared_ptr<SubObject>> result;
    for(const auto& object : objects) {
        if(const auto& object_cast = std::dynamic_pointer_cast<SubObject>(object)) {
