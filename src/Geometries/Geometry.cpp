...
void Geometry::update() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, _vbo);
    glBufferData(
        GL_ARRAY_BUFFER, _vertices.size() * sizeof(Vertex), &_vertices[0],
        GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ebo);
    glBufferData(
        GL_ELEMENT_ARRAY_BUFFER, _triangles.size() * sizeof(Triangle),
...
void Geometry::draw(const std::optional<size_t> &material_index) {
    ...
    glDrawElements(
        GL_TRIANGLES, count * 3, GL_UNSIGNED_INT,
        (void *)(start * sizeof(Triangle)));
    glBindVertexArray(0);
}
...
void Geometry::sortTriangles() {
    ...
    std::sort(
        _sorted_materials.begin(), _sorted_materials.end(),
